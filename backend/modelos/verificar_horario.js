eje = function(arrays,origen,redisClient) {
	return new Promise(function(resolve, reject) {	
		...
		/*
		    Recibo un array con token y tiempo (hh:mm)
		*/				
		if (arrays.length==2) {		
			...
			jwt.verify(arrays[0], 'clWve-G*-9)1', function(err, decoded) {		                	
				if (err) {
					...
					*/					
					redisClient.keys("configuracion_*", function (err, reply) {
                        if( reply.length > 0 ){
                            redisClient.get(reply[0], function (error, datosEmpresa){
                                const entidad = JSON.parse(datosEmpresa);
                                if(arrays[1] > entidad[17]) reject([false, "Se cerró la sesión, está fuera del horario establecido"]);
                                else resolve([true, reply]);
                            })
                        }						
					});						
				}else{
