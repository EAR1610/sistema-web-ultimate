eje = function(arrays,origen,redisClient) {
	return new Promise(function(resolve, reject) {
		...
		/*
			recibe token y correo del ayudante o supervisor asociado
		*/		
		if (arrays.length==2){		
			...
			jwt.verify(arrays[0], 'clWve-G*-9)1', function(err, decoded) {
				if (err) {
					...
					/*
						busca y elimina dicha relacion
					*/					
					redisClient.keys("relacion_usuario_*_"+arrays[1],function(err,reply) {
						redisClient.del(reply[0],function(err,replly) {
							resolve([true,true]);
						});
					});						
				}else{
