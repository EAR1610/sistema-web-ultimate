eje = function(arrays,origen,redisClient) {
	return new Promise(function(resolve, reject) {
		...
		/*
		    Recibo un array con solo nombre y password
		*/
		/*
		    al tener los registros los intero etilo kanban usando recursividad
		*/		
		if (arrays.length==2) {
            redisClient.keys("configuracion_*", function (errorConf, configuracion){
                if( configuracion !== null && configuracion !== undefined ){                    
                    redisClient.get(configuracion[0], function (err, reply) {
                        resolve([true, reply]);
                    });						
                }
            });
